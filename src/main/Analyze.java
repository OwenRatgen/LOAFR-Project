...
public class Analyze {
    ...
    // creates a sublist which contains only the rows found using the header and
    // keyword parameters
    public List<Map<String, Object>> findInstances(List<Map<String, Object>> data, String header, String keyword) {
        List<Map<String, Object>> newList = null;

        try {
            newList = data.stream().filter(map -> map.containsKey(header) && map.get(header).equals(keyword))
            ...
            System.out.println(
                    "An error occured while trying to find instance of specified data. Message: " + e.getMessage());
            e.printStackTrace();
        }
    ...
    /*
     * returns a list of maps that contains only the entries that are less than the
     * argument 'value' in
     * the column 'column'
     */
    ...
    /*
     * returns a list of maps that contains only the entries that are greater than
     * the
     * argument 'value' in
     * the column 'column'
     */
    ...
    // frequency analysis function. creates a hashMap that performs frequency
    // analysis on a list of strings
    public static Map<String, Integer> frequencyAnalysis(List<String> logEntries) {
        ...
        try {
            ...
            for (String entry : logEntries) {
                // You may need to preprocess the entry (e.g., remove whitespace, convert to
                // lowercase) based on your requirements
    ...
    public static Map<String, List<String>> dependencyMapping(List<String> logEntries) {
        ...
        try {
            ...
            for (String entry : logEntries) {
                ...
                // For simplicity, let's assume entries are in the format "ComponentA depends on
                // ComponentB"
            ...
            System.out.println(
                    "An error occured when trying to run dependency mapping analysis. Message: " + e.getMessage());
            e.printStackTrace();
        }
